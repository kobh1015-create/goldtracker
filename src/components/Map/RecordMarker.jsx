...
export default function RecordMarker({ record, onDelete }) {
  const dateStr = new Date(record.date).toLocaleDateString('ko-KR')
  const amountStr = `${record.amount}${record.unit}`
      ...
      <Popup minWidth={200}>
        <div>
          ...
          <div className="mt-1 space-y-0.5 text-xs text-gray-600">
            <p>📅 {dateStr}</p>
            <p>⚖️ 채취량: <span className="font-semibold text-amber-600">{amountStr}</span></p>
